...
const EditRentShort = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch('http://85.190.242.108:4483/api/RentCar/Short/Edit', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName={`Edit / ${cardNumber}`} prevPageName='Rent short orders' prevRoute='/car/short-orders' />
            {formOptions ? (
                        ...
                        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                            <form>
                                <div className="p-6.5">
                                    ...
                                    <div className='mb-3 flex flex-col gap-6 xl:flex-row'>
                                        <SelectGroupOne text="Service Type" options={[{ value: "M-000089", text: "Rent a Car Short" }]} setSelectedData={setSelectedData} disabled={false} defaultValue={selectedServiceType} isInvalid={invalidFields.includes('Service Type')}/>
                                        <div className="w-full xl:w-full">
                                            <label className="mb-2.5 block text-black dark:text-white">
                                                Driver
                                            </label>
