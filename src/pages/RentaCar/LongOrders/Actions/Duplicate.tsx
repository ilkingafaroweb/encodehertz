...
const DuplicateRentLong = () => {
    ...
    const addCarLong = async () => {
        await fetch('http://85.190.242.108:4483/api/RentCar/Long/Create', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName={`Duplicate`} prevPageName='Rent long orders' prevRoute='/car/long-orders' />
            {formOptions ? (
...
export default DuplicateRentLong;